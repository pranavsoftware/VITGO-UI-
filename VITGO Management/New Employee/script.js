...
async function generateUniqueEmail(name) {
    let email = `${name.toLowerCase().replace(/\s+/g, '')}@emp.vitgo`;
    ...
    if (!querySnapshot.empty) {
        ...
        while (!querySnapshot.empty) {
            email = `${name.toLowerCase().replace(/\s+/g, '')}${count}@emp.vitgo`;
...
document.getElementById('employeeForm').addEventListener('submit', async (e) => {
    ...
    const mobileNumber = document.getElementById('mobileNumber').value; // Added Mobile Number
    ...
    if (!/^\d{10}$/.test(mobileNumber)) {
        popupMessage.textContent = "Error: Mobile number must be exactly 10 digits.";
    ...
    try {
        ...
        if (user) {
            ...
            await setDoc(doc(db, 'TAXIemployee', user.uid), {
                ...
                email,
                age,
                mobileNumber, // 🔹 Storing Mobile Number
                address,
                licenceNumber,
