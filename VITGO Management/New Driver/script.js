...
onAuthStateChanged(auth, (user) => {
    if (!user || !user.email.endsWith("@emp.vitgo")) {
        window.location.href = "../VITGO Management/Login Page/"; // Redirect to login page if unauthorized
    }
});
...
async function generateUniqueEmail(baseEmail) {
    ...
    while (emailExists) {
        ...
        if (querySnapshot.empty) {
            ...
            uniqueEmail = baseEmail.replace('@driver.vitgo', '') + counter + '@driver.vitgo';
            counter++;
        }
    }
    return uniqueEmail;
...
document.getElementById('name').addEventListener('input', function () {
    ...
    if (nameValue) {
        const baseEmail = nameValue.replace(/\s+/g, '').toLowerCase() + '@driver.vitgo';
